...
func TestAccDataSourceCcWirelessProfilePolicyTag(t *testing.T) {
	var checks []resource.TestCheckFunc
	checks = append(checks, resource.TestCheckResourceAttr("data.catalystcenter_wireless_profile_policy_tag.test", "policy_tag_name", "PolicyTag1"))
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccDataSourceCcWirelessProfilePolicyTagPrerequisitesConfig + testAccDataSourceCcWirelessProfilePolicyTagConfig(),
...
const testAccDataSourceCcWirelessProfilePolicyTagPrerequisitesConfig = `
...
func testAccDataSourceCcWirelessProfilePolicyTagConfig() string {
	config := `resource "catalystcenter_wireless_profile_policy_tag" "test" {` + "\n"
	config += `	wireless_profile_id = catalystcenter_network_profile_for_sites_assignments.test.network_profile_id` + "\n"
	config += `	policy_tag_name = "PolicyTag1"` + "\n"
	config += `	site_ids = [catalystcenter_building.test_building.id]` + "\n"
	config += `}` + "\n"

	config += `
		data "catalystcenter_wireless_profile_policy_tag" "test" {
			id = catalystcenter_wireless_profile_policy_tag.test.id
			wireless_profile_id = catalystcenter_network_profile_for_sites_assignments.test.network_profile_id
			policy_tag_id = catalystcenter_wireless_profile_policy_tag.test.policy_tag_id
		}
	`
	return config
}
