...
func TestAccCcWirelessProfilePolicyTag(t *testing.T) {
	var checks []resource.TestCheckFunc
	checks = append(checks, resource.TestCheckResourceAttr("catalystcenter_wireless_profile_policy_tag.test", "policy_tag_name", "PolicyTag1"))

	var steps []resource.TestStep
	if os.Getenv("SKIP_MINIMUM_TEST") == "" {
		steps = append(steps, resource.TestStep{
			Config: testAccCcWirelessProfilePolicyTagPrerequisitesConfig + testAccCcWirelessProfilePolicyTagConfig_minimum(),
		})
	}
	steps = append(steps, resource.TestStep{
		Config: testAccCcWirelessProfilePolicyTagPrerequisitesConfig + testAccCcWirelessProfilePolicyTagConfig_all(),
...
const testAccCcWirelessProfilePolicyTagPrerequisitesConfig = `
...
func testAccCcWirelessProfilePolicyTagConfig_minimum() string {
...
func testAccCcWirelessProfilePolicyTagConfig_all() string {
	config := `resource "catalystcenter_wireless_profile_policy_tag" "test" {` + "\n"
	config += `	wireless_profile_id = catalystcenter_network_profile_for_sites_assignments.test.network_profile_id` + "\n"
	config += `	policy_tag_name = "PolicyTag1"` + "\n"
	config += `	site_ids = [catalystcenter_building.test_building.id]` + "\n"
